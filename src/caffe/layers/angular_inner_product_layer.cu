...
#include "caffe/layers/angular_inner_product_layer.hpp"
...
template <typename Dtype>
__global__ void Nomarlized_weight_gpu(int nthreads, const int K_,
	Dtype* w_norm, Dtype* weight) {
	CUDA_KERNEL_LOOP(index, nthreads) {
		for (int i = 0; i < K_; i++) {
			weight[index * K_ + i] /= w_norm[index];
		}
		w_norm[index] = (Dtype)1.;
	}
}
template <typename Dtype>
...
template <typename Dtype>
__global__ void Angular_double_forward_gpu(int nthreads, const int N_, Dtype lambda,
...
template <typename Dtype>
__global__ void Angular_triple_forward_gpu(int nthreads, const int N_, Dtype lambda,
...
template <typename Dtype>
__global__ void Angular_quadruple_forward_gpu(int nthreads, const int N_, Dtype lambda,
...
template <typename Dtype>
__global__ void Angular_weight_double_backward_gpu(int nthreads, const int M_, 
...
template <typename Dtype>
__global__ void Angular_weight_triple_backward_gpu(int nthreads, const int M_, 
            const int N_, const int K_, Dtype lambda, const Dtype* weight,
            const Dtype* bottom, const Dtype* top_diff, const Dtype* label,
...
template <typename Dtype>
__global__ void Angular_weight_quadruple_backward_gpu(int nthreads, const int M_, 
            const int N_, const int K_, Dtype lambda, const Dtype* weight,
            const Dtype* bottom, const Dtype* top_diff, const Dtype* label,
...
template <typename Dtype>
__global__ void Angular_bottom_double_backward_gpu(int nthreads, const int N_, const int K_, Dtype lambda,
...
template <typename Dtype>
__global__ void Angular_bottom_triple_backward_gpu(int nthreads, const int N_, const int K_, Dtype lambda,
            const Dtype* bottom, const Dtype* weight, const Dtype* top_diff, const Dtype* label,
            const Dtype* x_norm, const Dtype* w_norm, const Dtype* sign_1, const Dtype* sign_2, 
...
template <typename Dtype>
__global__ void Angular_bottom_quadruple_backward_gpu(int nthreads, const int N_, const int K_, Dtype lambda,
            const Dtype* bottom, const Dtype* weight, const Dtype* top_diff, const Dtype* label,
            const Dtype* x_norm, const Dtype* w_norm, const Dtype* sign_3, 
            const Dtype* sign_4, const Dtype* cos_theta, const Dtype* cos_theta_quadratic, 
...
template <typename Dtype>
void AngularInnerProductLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  iter_ += (Dtype)1.;
  Dtype base_ = this->layer_param_.angular_inner_product_param().base();
  Dtype gamma_ = this->layer_param_.angular_inner_product_param().gamma();
  Dtype power_ = this->layer_param_.angular_inner_product_param().power();
  Dtype lambda_min_ = this->layer_param_.angular_inner_product_param().lambda_min();
  ...
  // |w| = 1
  nthreads = N_;
  Nomarlized_weight_gpu<Dtype> << <CAFFE_GET_BLOCKS(nthreads),
	  CAFFE_CUDA_NUM_THREADS >> >(nthreads, K_, w_norm_.mutable_gpu_data(),
		  mutable_weight);
  /*Dtype * data = new Dtype[N_ *K_];
  std::vector<Dtype> vec_data;
  cudaMemcpy(data, weight, N_*K_ * sizeof(Dtype), cudaMemcpyDeviceToHost);
  ...
  /************************* optional variables *************************/
  switch (type_) {
  case AngularInnerProductParameter_AngularType_SINGLE:
    break;
  case AngularInnerProductParameter_AngularType_DOUBLE:
    // cos_theta_quadratic
    caffe_gpu_powx(M_ * N_, cos_theta_.gpu_data(), (Dtype)2., cos_theta_quadratic_.mutable_gpu_data());
    break;
  case AngularInnerProductParameter_AngularType_TRIPLE:
    ...
    break;
  case AngularInnerProductParameter_AngularType_QUADRUPLE:
    ...
    break;
  default:
    LOG(FATAL) << "Unknown Angular type.";
  }
  ...
  case AngularInnerProductParameter_AngularType_DOUBLE:
    ...
    Angular_double_forward_gpu<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
    ...
    break;
  case AngularInnerProductParameter_AngularType_TRIPLE:
    Angular_triple_forward_gpu<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
      CAFFE_CUDA_NUM_THREADS>>>(nthreads, N_, lambda_, label, x_norm_.gpu_data(), w_norm_.gpu_data(),
                                sign_1_.gpu_data(), sign_2_.gpu_data(), cos_theta_.gpu_data(), 
                                cos_theta_cubic_.gpu_data(), top_data);
    break;
  case AngularInnerProductParameter_AngularType_QUADRUPLE:
    Angular_quadruple_forward_gpu<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
      CAFFE_CUDA_NUM_THREADS>>>(nthreads, N_, lambda_, label, x_norm_.gpu_data(), w_norm_.gpu_data(),
...
template <typename Dtype>
void AngularInnerProductLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[0]) {
    ...
    case AngularInnerProductParameter_AngularType_SINGLE:
      ...
      break;
    case AngularInnerProductParameter_AngularType_DOUBLE:
      Angular_weight_double_backward_gpu<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
      ...
      break;
    case AngularInnerProductParameter_AngularType_TRIPLE:
      Angular_weight_triple_backward_gpu<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
        CAFFE_CUDA_NUM_THREADS>>>(nthreads, M_, N_, K_, lambda_, weight, bottom_data, top_diff, label,
      ...
      break;
    case AngularInnerProductParameter_AngularType_QUADRUPLE:
      Angular_weight_quadruple_backward_gpu<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
        CAFFE_CUDA_NUM_THREADS>>>(nthreads, M_, N_, K_, lambda_, weight, bottom_data, top_diff, label,
  ...
  if (propagate_down[0]) {
    ...
    case AngularInnerProductParameter_AngularType_SINGLE:
      ...
      break;
    case AngularInnerProductParameter_AngularType_DOUBLE:
      Angular_bottom_double_backward_gpu<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
      ...
      break;
    case AngularInnerProductParameter_AngularType_TRIPLE:
      Angular_bottom_triple_backward_gpu<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
        CAFFE_CUDA_NUM_THREADS>>>(nthreads, N_, K_, lambda_, bottom_data, weight, top_diff, label,
                                  x_norm_.gpu_data(), w_norm_.gpu_data(), sign_1_.gpu_data(), sign_2_.gpu_data(),
      ...
      break;
    case AngularInnerProductParameter_AngularType_QUADRUPLE:
      Angular_bottom_quadruple_backward_gpu<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
        CAFFE_CUDA_NUM_THREADS>>>(nthreads, N_, K_, lambda_, bottom_data, weight, top_diff, label,
                                  x_norm_.gpu_data(), w_norm_.gpu_data(), sign_3_.gpu_data(), sign_4_.gpu_data(),
                                  cos_theta_.gpu_data(), cos_theta_quadratic_.gpu_data(),
...
INSTANTIATE_LAYER_GPU_FUNCS(AngularInnerProductLayer);
