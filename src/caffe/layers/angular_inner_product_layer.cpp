...
void AngularInnerProductLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  type_ = this->layer_param_.angular_inner_product_param().type();
  iter_ = this->layer_param_.angular_inner_product_param().iteration();
  lambda_ = (Dtype)0.;

  const int num_output = this->layer_param_.angular_inner_product_param().num_output();
  N_ = num_output;
  const int axis = bottom[0]->CanonicalAxisIndex(
      this->layer_param_.angular_inner_product_param().axis());
  ...
  if (this->blobs_.size() > 0) {
    ...
    shared_ptr<Filler<Dtype> > weight_filler(GetFiller<Dtype>(
        this->layer_param_.angular_inner_product_param().weight_filler()));
    weight_filler->Fill(this->blobs_[0].get());
	//
	//normalized_weight = new Dtype[N_*K_];
  }  // parameter initialization
  this->param_propagate_down_.resize(this->blobs_.size(), true);
}

template <typename Dtype>
void AngularInnerProductLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  case AngularInnerProductParameter_AngularType_QUADRUPLE:
    ...
    break;
  default:
    LOG(FATAL) << "Unknown Angular-Softmax type.";
  }
}

template <typename Dtype>
void AngularInnerProductLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  iter_ += (Dtype)1.;
...
template <typename Dtype>
void AngularInnerProductLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(AngularInnerProductLayer);
#endif

INSTANTIATE_CLASS(AngularInnerProductLayer);
REGISTER_LAYER_CLASS(AngularInnerProduct);
